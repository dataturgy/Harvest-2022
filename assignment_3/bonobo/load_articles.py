...
@use('postgres_app')
def load_articles(article_nr, article_name, article_price, article_warehouse, article_shelf, **args):
    try:
        cursor = args['postgres_app'].cursor()
        cursor.execute("INSERT INTO article (article_nr, article_name, article_price, article_warehouse, article_shelf) VALUES(%s, %s, %s, %s, %s)", 
        (article_nr, article_name, article_price, article_warehouse, article_shelf))
...
def get_graph(**options):
    ...
    graph = bonobo.Graph()
    graph.add_chain(
        bonobo.CsvReader('article.csv', fs='fs.data'),
        load_articles
    )
