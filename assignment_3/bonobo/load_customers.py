...
@use('postgres_app')
def load_customer(customer_nr, customer_name, customer_address ,customer_city, **args):
    try:
        cursor = args['postgres_app'].cursor()
        cursor.execute("INSERT INTO customer (customer_nr, customer_name, customer_address ,customer_city) VALUES(%s, %s, %s, %s)", 
        (customer_nr, customer_name, customer_address ,customer_city))
...
def get_graph(**options):
    ...
    graph = bonobo.Graph()
    graph.add_chain(
         bonobo.CsvReader('customer.csv', fs='fs.data'),
         load_customer
    )
    return graph
